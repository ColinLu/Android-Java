...
public class ProgressDrawable extends Drawable implements Animatable {
    ...
    private static final int[] COLORS = new int[]{Color.BLACK, Color.WHITE};

    private static final float RADIUS_LARGE = 11f;
    ...
    public void setStyle(@ProgressStyle int style) {
        if (style == DEFAULT) setSize(RADIUS, STROKE_WIDTH, ARROW_WIDTH, ARROW_HEIGHT);
        else setSize(RADIUS_LARGE, STROKE_WIDTH_LARGE, ARROW_WIDTH_LARGE, ARROW_HEIGHT_LARGE);
    }

    public void setSize(float radius, float strokeWidth, float arrowWidth, float arrowHeight) {
        final Ring ring = mRing;
        ring.setCenterRadius(radius * mDensity);
        ring.setStrokeWidth(strokeWidth * mDensity);
        ring.setColorIndex(0);
        ring.setArrow(arrowWidth * mDensity, arrowHeight * mDensity);
        invalidateSelf();
    }

    public void setStrokeWidth(@Px float width) {
        if (mRing.getStrokeWidth() != width) {
            mRing.setStrokeWidth(width);
            invalidateSelf();
        }
    }

    public void setCenterRadius(float radius) {
        if (mRing.getCenterRadius() != radius) {
            mRing.setCenterRadius(radius);
            invalidateSelf();
        }
    }
    ...
    public void setStrokeCap(@NonNull Paint.Cap strokeCap) {
        if (!Objects.equals(strokeCap, mRing.getStrokeCap())) {
     ...
     */
    public void setArrow(float width, float height) {
        mRing.setArrow(width, height);
        invalidateSelf();
    }
    ...
    private static class Ring {
         ...
         */
        void setArrow(float width, float height) {
